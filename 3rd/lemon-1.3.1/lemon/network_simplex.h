...
namespace lemon {
  ...
  public:
    ...
    NetworkSimplex(const GR& graph, bool arc_mixing = true) :
      ...
      MAXVAL(std::numeric_limits<Value>::max()),
      INF(std::numeric_limits<Value>::has_infinity ?
          std::numeric_limits<Value>::infinity() : MAXVAL)
    {
  ...
  private:
    ...
    bool init() {
      ...
      if (_has_lower) {
        for (int i = 0; i != _arc_num; ++i) {
          Value c = _lower[i];
          if (c >= 0) {
            _cap[i] = _upper[i] < MAXVAL ? _upper[i] - c : INF;
          } else {
            _cap[i] = _upper[i] < MAXVAL + c ? _upper[i] - c : INF;
          }
          _supply[_source[i]] -= c;
          _supply[_target[i]] += c;
        }
    ...
    ProblemType start() {
      ...
      while (pivot.findEnteringArc()) {
        findJoinNode();
        bool change = findLeavingArc();
        if (delta >= MAXVAL) return UNBOUNDED;
        changeFlow(change);
        if (change) {
          updateTreeStructure();
          updatePotential();
